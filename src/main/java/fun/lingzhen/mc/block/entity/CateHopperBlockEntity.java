...
public class CateHopperBlockEntity extends LootableContainerBlockEntity implements Hopper {
    ...
    public CateHopperBlockEntity(BlockPos pos, BlockState state) {
        super(ModBlockEntityTypes.CATE_HOPPER, pos, state);
    ...
    protected Text getContainerName() {
        return Text.translatable("container.cate_hopper");
    }

    public static void serverTick(World world, BlockPos pos, BlockState state, CateHopperBlockEntity blockEntity) {
    ...
    private static boolean insertAndExtract(World world, BlockPos pos, BlockState state, CateHopperBlockEntity blockEntity, BooleanSupplier booleanSupplier) {
        if (world.isClient) {
            return false;
        } else {
    ...
    private static boolean insert(World world, BlockPos pos, CateHopperBlockEntity blockEntity) {
    ...
    public static boolean extract(World world, Hopper hopper) {
        ...
        if (inventory != null) {
            ...
            if (!bl) {
                java.util.Iterator<ItemEntity> var6 = getInputItemEntities(world, hopper).iterator();
    ...
    private static boolean extract(Hopper hopper, Inventory inventory, int slot, Direction side) {
        ...
        if (!itemStack.isEmpty() && canExtract(hopper, inventory, itemStack, slot, side)) {
            ...
            ItemStack itemStackTrans = hopper.getStack(hopper.size() - 1);
...
//        修正 最后 一个 不能 自动移入
    ...
    private static ItemStack transfer(@Nullable Inventory from, Inventory to, ItemStack stack, int slot, @Nullable Direction side) {
        ...
        if (canInsert(to, stack, slot, side)) {
            ...
            if (bl) {
                if (bl2 && to instanceof CateHopperBlockEntity) {
                    CateHopperBlockEntity hopperBlockEntity = (CateHopperBlockEntity)to;
                    if (!hopperBlockEntity.isDisabled()) {
                        int j = 0;
                        if (from instanceof CateHopperBlockEntity) {
                            CateHopperBlockEntity hopperBlockEntity2 = (CateHopperBlockEntity)from;
    ...
    @Nullable
    private static Inventory getOutputInventory(World world, BlockPos pos, CateHopperBlockEntity blockEntity) {
    ...
    public static void onEntityCollided(World world, BlockPos pos, BlockState state, Entity entity, CateHopperBlockEntity blockEntity) {
    ...
    protected ScreenHandler createScreenHandler(int syncId, PlayerInventory playerInventory) {
        return new CateHopperScreenHandler(syncId, playerInventory, this);
    }
}
